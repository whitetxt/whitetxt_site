...
function openDefrag(file = null) {
  ...
  new_defrag.addEventListener("click", function() {
    focus_window(this.dataset.window_id);
  }.bind(new_defrag));

  new_defrag.querySelector("div.start").addEventListener("click", function() {
    start_defrag(this.dataset.window_id);
  }.bind(new_defrag));

  new_defrag.querySelector("div.quit").addEventListener("click", function() {
  ...
  create_taskbar_item("Disk Defragmenter", "assets/appicons/defrag.png", last_window_id);
...
function start_defrag(window_id) {
  ...
  setTimeout(() => {
    ...
    windows_open[window_id].querySelector("span.data").style.display = "inline-block";
    windows_open[window_id].querySelector("span.defrag-prog").style.display = "inline-block";
    windows_open[window_id].querySelector("span.status").innerHTML = "";
    defrag_windows[window_id] = {"completed": [], "length": 5, "snake": [[0, 0], [0, 0], [1, 0], [2, 0], [3, 0]], "direction": 0, "quit": false, "fruit": [20, 10]};
    ...
    for (var i = 0; i < squares.length; i++) {
      ...
      if (i == 420) {
        ...
        defrag_windows[window_id]["completed"].push([i % 40, Math.floor(i / 40)]);
      }
    }
...
function perform_defrag(window_id) {
  ...
  var head = defrag_windows[window_id]["snake"][defrag_windows[window_id]["snake"].length - 1];
  var new_head = [...head];
  ...
  if (snake.some(a => new_head.every((v, i) => v === a[i]))) {
    ...
    windows_open[window_id].querySelector("span.defrag-prog").style.display = "none";
    windows_open[window_id].querySelector("span.status").classList.remove("win");
    windows_open[window_id].querySelector("span.status").classList.add("lose");
    windows_open[window_id].querySelector("span.status").innerHTML = "Defragmentation Failed:<br>Hard Drive Disk Platters Collapsed.";
    return;
  }
  defrag_windows[window_id]["snake"].push(new_head);
  ...
  if (snake.some(a => fruit.every((v, i) => v === a[i]))) {
    ...
    if (indexes.length === 0) {
      ...
      windows_open[window_id].querySelector("span.defrag-prog").innerText = `${Math.floor((num_comp * 100) / (40 * 20))}%`;
      for (var i = 0; i < squares.length; i++) {
        squares[i].classList.add("win");
        squares[i].classList.remove("dead");
        squares[i].classList.remove("snake");
        squares[i].classList.remove("completed");
        squares[i].style.animationDelay = `-${Math.round(Math.random() * 50) * 10}ms`;
      }
      windows_open[window_id].querySelector("span.data").style.display = "none";
      windows_open[window_id].querySelector("span.defrag-prog").style.display = "none";
      windows_open[window_id].querySelector("span.status").innerHTML = "Defragmentation has been completed!";
      windows_open[window_id].querySelector("span.status").classList.add("win");
      windows_open[window_id].querySelector("span.status").classList.remove("lose");
      return;
    } else {
      ...
      defrag_windows[window_id]["snake"].unshift([...defrag_windows[window_id]["snake"][0]]);
